// medbasha - Lógica de BusTalk v2.9.24_GPS
...
function busTalkApp() {
    return {
        ...
        // AUTO "EN PARADA" OPTIMIZADO (RADIO AMPLIADO A 75m Y UBRAL 6 KM/H)
        comprobarLlegadaMarquesina(lat, lng, velocidadKmh) {
            let siguienteDestino = this.obtenerDestinoSiguiente();

            for (let parada in this.coordenadasParadas) {
                let coords = this.coordenadasParadas[parada];
                let dist = this.calcularDistanciaMetros(lat, lng, coords.lat, coords.lng);
                
                // Radio ampliado a 75 metros para garantizar captura
                if (dist < 75 && this.misParadasArray.includes(parada)) {
                    
                    // Caso A: Pasa de largo sin detenerse (> 15 km/h)
                    if (velocidadKmh > 15) {
                        this.segundosParadoAutoGPS = 0;
                        if (this.miUbicacion !== parada) {
                            this.actualizarUbicacion(parada);
                            setTimeout(() => {
                                if (this.enParada) this.toggleParada();
                            }, 100);
                        }
                    } 
                    // Caso B: Está detenido cerca de la parada (velocidad < 6 km/h)
                    else if (velocidadKmh < 6) {
                        this.segundosParadoAutoGPS++;
                        // Requiere solo 2 lecturas seguidas
                        if (this.segundosParadoAutoGPS >= 2) {
                            if (this.miUbicacion !== parada) {
                                this.actualizarUbicacion(parada);
                            } else if (!this.enParada) {
                                this.enParada = true;
                                this.etaFinal = null;
                                this.miTramoActual = '';
            ...
            // RESPALDO DE SEGURIDAD: Si la cuenta atrás llegó a 0 y lleva 10 segs parado cerca del destino
            if (!this.enParada && velocidadKmh < 6 && siguienteDestino) {
                let coordsDestino = this.coordenadasParadas[siguienteDestino];
                if (coordsDestino) {
                    let dDestino = this.calcularDistanciaMetros(lat, lng, coordsDestino.lat, coordsDestino.lng);
                    if (dDestino < 120) {
